...
public class FilterByStatus {
	...
	// SeachByStatus
	@Parameters({ "strPath" })
	public static void FilterStatus(WebDriver wd, String pr, String dt, String strTCNum, String strStep,
		...
		try {
			...
			for (WebElement status : val) {
				String btnst = status.getAttribute("class");
				boolean btn = btnst.contains("nonbtndisable");
				System.out.println("the button is " + btn);
				System.out.println("Values..... " + status.getText());
				String code = status.getText();
				System.out.println("Code is........." + code);
				if (code.contains(dt) && btn == false) {
					...
					break;
				} else if (code.contains(dt) && btn == true) {
					String Methodname = Thread.currentThread().getStackTrace()[1].getMethodName();
					GenerateReport.getmethodname(Methodname, VolPayDriver.scenario, strStep);
					st.Pass(wd, pr, strTCNum, strStep, ScenarioNum, strPath);
					break;
