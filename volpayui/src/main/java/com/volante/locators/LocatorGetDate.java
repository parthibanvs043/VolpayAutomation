...
public class LocatorGetDate {
	public static void SelectDate(WebDriver wd, String pr, String strStep) throws InterruptedException, IOException {		
		try {
			boolean isChecked = false;
			wd.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		wd.findElements(By.cssSelector(pr)).get(0).click();
			WebElement alll = wd.findElement(By.className("table-condensed"));
			((JavascriptExecutor) wd).executeScript("arguments[0].scrollIntoView(true);", alll);
