...
public class DistributionDataCurrencyCorrespondent
{
	 public static Workbook BRBook;
	   static Sheet BRSheet;

public static void ExecuteCurrencyCorrespondent(WebDriver wd,String Sheetname , String destFile) throws Exception 
	{
		    try
		    {
		    ...
		    WebElement CurrencyCorrespondent = wait.until(ExpectedConditions.visibilityOfElementLocated(By.linkText(DistributionList.CurrencyCorrespondentLinkVal)));
		    ...
		    for (int row=1; row<=newrowscount; row++)
		       {    
			        ...
			        ReportGeneration.test = ReportGeneration.extent.createTest(TestName.getContents()).assignCategory("CurrencyCorrespondent");
	       	        Cell CurrencyCode=BRSheet.getCell(2,row);
	                Cell CurrencyCorrespondentPartyCode=BRSheet.getCell(3,row);
	               ...
	               // Cell EffectiveFromDate=BRSheet.getCell(7,row);
	                ...
	                Thread.sleep(2000);
	                LocatorByXPATH.Select_XPath(wd, DistributionList.CCCurrencyCodeVal, CurrencyCode.getContents(), "CurrencyCode");
	                Thread.sleep(2000);
	                LocatorByXPATH.Select_XPath(wd, DistributionList.CurrencyCorrespondentPartyCodeVal, CurrencyCorrespondentPartyCode.getContents(), "CurrencyCorrespondentPartyCode");
	                Thread.sleep(3000);
	               LocatorByXPATH.DropDown_Text_XPath(wd, DistributionList.RoutingMethodVal, RoutingMethod.getContents(), "RoutingMethod");             
	                LocatorByID.Enter_ID(wd, DistributionList.CcyCMinimumProcessingDaysVal, MinimumProcessingDays.getContents(), "MinimumProcessingDays");
	                Thread.sleep(3000);
	                ...
	                LocatorByLinks.Click_Link(wd, DistributionList.CurrencyCorrespondentLinkVal, "");
	                ChangesLostPopUP.ChangesLost(wd,"CurrencyCorrespondent ");
	                Thread.sleep(2000);
	           }
		    }
		    catch (IOException e)
